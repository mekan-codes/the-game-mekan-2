...
"%GCC%" -std=c99 -Wall -Wextra -O2 %IMAGE_CFLAGS% src\crane_game.c -o crane_game.exe -I"%SDL_PREFIX%\include" -L"%SDL_PREFIX%\lib" %IMAGE_LIBS% -lSDL2_ttf -lSDL2 -lm -static-libgcc
if errorlevel 1 (
    echo Build failed.
    exit /b 1
)
...
if "%IMAGE_ENABLED%"=="1" (
    rem SDL2_image depends on additional MSYS2 runtime DLLs. Keep fallback copies in runtime; do not clutter repo root.
    for %%D in (
        ...
        if not exist "runtime\%%D" if exist "%SDL_PREFIX%\bin\%%D" (
            copy /Y "%SDL_PREFIX%\bin\%%D" "runtime\%%D" >nul
        )
    )
) else (
    rem Keep fallback DLLs in runtime; do not clutter repo root.
    if not exist "runtime\SDL2.dll" if exist "%SDL_PREFIX%\bin\SDL2.dll" copy /Y "%SDL_PREFIX%\bin\SDL2.dll" "runtime\SDL2.dll" >nul
    if not exist "runtime\SDL2_ttf.dll" if exist "%SDL_PREFIX%\bin\SDL2_ttf.dll" copy /Y "%SDL_PREFIX%\bin\SDL2_ttf.dll" "runtime\SDL2_ttf.dll" >nul
    if not exist "runtime\SDL2_image.dll" if exist "%SDL_PREFIX%\bin\SDL2_image.dll" copy /Y "%SDL_PREFIX%\bin\SDL2_image.dll" "runtime\SDL2_image.dll" >nul
)

echo.
echo Build complete: crane_game.exe
echo Run with:
echo   Play_Crane_Game.bat
echo.
echo Runtime DLL fallbacks are kept in runtime\ to keep the repo root clean.
